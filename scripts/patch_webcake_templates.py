...
(function(){
  ...
  function applyViewport(){
    var w=window.innerWidth||document.documentElement.clientWidth||CW;
    var mobile=w<768;
    window.DISPLAY=mobile?"mobile":"desktop";
    var c=mobile
      ? ("width="+CW+", user-scalable=no, initial-scale="+(w/CW)+", minimum-scale="+(w/CW)+", maximum-scale="+(w/CW)+", viewport-fit=cover")
      : ("width="+DW+", user-scalable=no, initial-scale=1.0");
  ...
  applyViewport();
  window.addEventListener("resize",function(){clearTimeout(window.__wedVp);window.__wedVp=setTimeout(applyViewport,120);});
